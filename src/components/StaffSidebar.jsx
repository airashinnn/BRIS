...
const StaffSidebar = ({ staffName = "Maria Santos" }) => {
  const navigate = useNavigate();

  const staffLinks = [
    { name: "Dashboard", path: "/staff/", icon: <LayoutDashboard size={20} /> },
    { name: "Requests", path: "/staff/requests", icon: <FileText size={20} /> },
    { name: "Residents", path: "/staff/residents", icon: <Users size={20} /> },
    { name: "Households", path: "/staff/households", icon: <Home size={20} /> },
    { name: "Announcements", path: "/staff/announcements", icon: <Megaphone size={20} /> },
    { name: "Activity Log", path: "/staff/activity-log", icon: <History size={20} /> },
  ];
  ...
  return (
    <div className="w-64 bg-gradient-to-b from-white to-gray-50 border-r border-gray-200 h-screen fixed left-0 top-0 overflow-y-auto shadow-lg">
      ...
      <div className="mx-4 mt-4 p-4 bg-emerald-50 rounded-xl border border-emerald-100">
        <p className="text-xs text-emerald-600 font-medium mb-1">Staff Portal</p>
        <p className="font-semibold text-gray-800 flex items-center gap-2">
          <span className="w-2 h-2 bg-emerald-500 rounded-full"></span>
          {staffName}
        </p>
        <p className="text-xs text-gray-500 mt-1">Secretary</p>
      </div>
